...
client.writeFile = (path, object) => {
	if (Object.entries(object).length ===
	    0) throw new Error('L\'object que vous voulez enregistrer a un problème et est vide.\nPar sécurité une erreur a été créée et le fichier n\'a donc pas été enregistré.');
...
fs.readdir('./commands/', (err, files) => {
	...
	numberFiles += files.length;
	console.log(chalk.white(`Chargement total de `) +
	            chalk.magenta.bold(`${numberFiles}`) +
	            chalk.white(` fichiers dont ${chalk.magenta.bold(commands)} commandes et ${chalk.magenta.bold(events)} évènements.`));
});
